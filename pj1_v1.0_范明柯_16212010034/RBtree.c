...
///红黑树删除结点调整函数
void RB_DELETE_FIXUP(Root *root,Node *x,Node *parent)
{
   ...
   if(node != NULL)
    {
        ...
        //a[m] = node->word;
        if(strcmp(deleteword,node->word) == 0)
            deleteNode = node;
...
int main()
{
    int a[] = {10, 10, 10, 10, 10, 10, 20, 50, 80,45,56,78,22,33,44,55,66,36,47,58,69,70,99};
    char b[][9] = {"a","abandon","avoid","amuse","bind","bike","beyond","clone","click","lack","orange","mouse","the","hit","hack","mountain","angry","shoot","nickname","his","her","touch"};
   ...
   while(j++ < N)
   {
    ...
    case 3:
        ...
        fscanf(fp,"%s",word);
        fscanf(fp,"%d",&k);
        inorder_DeleteNode(root->node,word);
